...
console.log("File JS chi tiết sản phẩm đã chạy (Phiên bản sửa giữ nguyên gốc)!");
...
function renderThumbnails(product) {
    ...
    list.style.cssText = "display:flex; gap:10px; margin-top:15px; width: 100%; justify-content: space-between;";
...
function loadReviews() {
    ...
    if (productReviews.length === 0) {
        reviewListEl.innerHTML = "<p class='text-muted text-center py-3'>Chưa có đánh giá nào. Hãy là người đầu tiên!</p>";
        return;
    }
    ...
    const renderStars = (num) => {
        let html = '';
        for (let i = 1; i <= 5; i++) {
            if (i <= num) html += '<i class="fas fa-star text-warning"></i>'; // Sao vàng
            else html += '<i class="far fa-star text-secondary"></i>'; // Sao rỗng
        }
        return html;
    };

    reviewListEl.innerHTML = productReviews.reverse().map(r => `
        <div class="review-item border-bottom mb-3 pb-3">
            <div class="d-flex justify-content-between align-items-center mb-1">
                <strong style="font-size: 1.1rem;">${r.name}</strong>
                <small class="text-muted" style="font-size: 0.85rem">${r.date}</small>
            </div>
            <div class="mb-2">
                ${renderStars(r.stars || 5)} 
            </div>
            <p class="mb-0 text-secondary" style="white-space: pre-wrap;">${r.content}</p>
        </div>
    `).join("");
}
...
function bindReviewEvents() {
    ...
    submitReviewBtn.onclick = function(e) {
        ...
        if (!nameInput || !contentInput) {
            alert("Lỗi HTML: Không tìm thấy ô nhập liệu (Kiểm tra lại ID review-name hoặc review-text)");
            return;
        }
...
function seedFakeReviews() {
    ...
    const fakeReviews = [
        {
            name: "Nguyễn Thu Hà",
            content: "Hoa bên ngoài nhìn tươi và đẹp hơn trong ảnh nhiều. Shop gói giấy màu pastel nhìn rất sang. Giao hàng đúng giờ hẹn. Sẽ ủng hộ shop dài dài ❤️",
            offset: 1000 * 60 * 60 * 2 
        },
        {
            name: "Trần Văn Nam",
            content: "Đặt tặng bạn gái nhân dịp kỷ niệm, cô ấy rất thích. Cảm ơn shop đã tư vấn nhiệt tình và viết thiệp giúp mình.",
            offset: 1000 * 60 * 60 * 24 * 2 
        },
        {
            name: "Lê Thị Bích Ngọc",
            content: "Hoa tươi, mix màu rất hài hòa. Tuy nhiên giao hàng hơi chậm hơn dự kiến 15 phút do tắc đường, nhưng bù lại anh shipper rất lễ phép.",
            offset: 1000 * 60 * 60 * 24 * 5 
        },
        {
            name: "Phạm Hoàng Anh",
            content: "Đã mua ở đây lần thứ 3 và chưa bao giờ thất vọng. 10 điểm cho chất lượng!",
            offset: 1000 * 60 * 60 * 24 * 10 //
        }
    ];

    // Lấy danh sách cũ
    let currentReviews = JSON.parse(localStorage.getItem("product_reviews") || "[]");

    // Thêm data mới vào
    fakeReviews.forEach((review, index) => {
        // Kiểm tra xem đã có đánh giá này chưa để tránh spam khi reload
        const isExist = currentReviews.some(r => r.productId == currentId && r.content === review.content);
...
function renderRelatedProducts(product) {
    ...
    let related = allProductsData.filter(p => p.category === product.category && p.id != product.id);

    // 2. [MỚI] Nếu ít hơn 4 sản phẩm, lấy thêm từ các danh mục khác bù vào
    if (related.length < 4) {
        // Lấy tất cả sản phẩm khác loại
        const otherProducts = allProductsData.filter(p => p.category !== product.category && p.id != product.id);
    ...
    if (related.length === 0) { 
        con.innerHTML = "<p class='text-center text-muted w-100'>Không có sản phẩm liên quan.</p>"; 
        return; 
    }
